...
#define CHOOSE_HASHMAP_MACRO(p_1, p_2, p_3, p_4, p_Macro, ...) CHOOSE_MACRO4(p_1, p_2, p_3, p_4, p_Macro, __VA_ARGS__)
#define CHOOSE_HM_INIT_MACRO(p_1, p_2, p_3, p_4, p_Macro, ...) CHOOSE_MACRO4(p_1, p_2, p_3, p_4, p_Macro, __VA_ARGS__)

#define HM_ELEMENT(p_Name, p_EType) struct { \
    uint8_t m_HasData: 1;                        \
    p_EType m_Value;                             \
} p_Name
...
#define S_HASHMAP_COMMONS(p_KType, p_EType, p_MaxSize) \
    ...
    uint8_t m_Occ[p_MaxSize];  /* Used for checking if position at index is free */ \
    struct {                                           \
        p_KType m_Key;                                 \
        p_EType m_El;                                  \
        } m_Data[p_MaxSize]

#define HASHMAP2(p_Name, p_KType, p_EType) struct { \
    S_HASHMAP_COMMONS(p_KType, p_EType, HM_MAX_SIZE); \
} p_Name

#define HASHMAP3(p_Name, p_KType, p_EType, p_MaxSize) struct { \
    S_HASHMAP_COMMONS(p_KType, p_EType, p_MaxSize);            \
} p_Name

#define S__INIT_HASHMAP_DEFAULT(p_Name, p_HashFn, p_MaxSize) do { \
    uint32_t hmSize = sizeof(p_Name.m_Data) / sizeof(p_Name.m_Data[0]); \
                                                                  \
    if (hmSize < p_MaxSize) {                                     \
        fprintf(stderr, "[%s!line %d]: Cannot init hashmap because of a mismatch of size. ", #p_Name, __LINE__); \
...
#define HASHMAP_PUT(p_Name, p_Key, p_El) do { \
    ...
    if (p_Name.m_Occ[idx] == HM_OCCUPIED) {   \
        fprintf(stderr, "Element position is occupied. Exiting program.\n"); \
        exit(1);                              \
    }                                         \
                                              \
    p_Name.m_CurrSize++;                      \
...
#define HASHMAP_INSERT(p_Name, p_Key, p_El) do { \
    if (p_Name.m_HashFn == NULL) {            \
        fprintf(stderr, "[%s]: Hash function is not initialized.\n", #p_Name); \
        exit(1);                              \
    }                                         \
...
#define HASHMAP_GET(p_Name, p_Key, p_Result) do { \
    uint64_t idx = p_Name.m_HashFn(p_Key) % p_Name.m_MaxSize; \
                                                  \
    pthread_spin_lock(&p_Name.m_Lock);            \
    if (p_Name.m_Occ[idx] == HM_NOT_OCCUPIED) {   \
...
#define HASHMAP_REMOVE(p_Name, p_Key, p_Result) do { \
    uint64_t idx = p_Name.m_HashFn(p_Key) % p_Name.m_MaxSize; \
    \
    pthread_spin_lock(&p_Name.m_Lock);            \
    if (p_Name.m_Occ[idx] == HM_NOT_OCCUPIED) {   \
...
#define HASHMAP(...) CHOOSE_HASHMAP_MACRO(__VA_ARGS__, HASHMAP3, HASHMAP2)(__VA_ARGS__)
#define INIT_HASHMAP(...) \
        CHOOSE_HM_INIT_MACRO(__VA_ARGS__, INIT_HASHMAP_BLOCK, INIT_HASHMAP_NON_BLOCK, INIT_HASHMAP_DEFAULT)(__VA_ARGS__)
